...
{
    public class SeasonCutOffDateService(IUnitOfWork unitOfWork, IMapper mapper,  ILogger<SeasonCutOffDateService> logger) : ISeasonCutOffDateService
    {
        ...
        /*
         * Takes a crop name and season name, then validates if today's date is after the cutoff date for the crop and season.
         */
        public async Task<ValidateApplicationDateResponse> ValidateApplicationDate(String cropName, String seasonName, String seasonYear, String regionName, CancellationToken cancellationToken)
        {
            ArgumentNullException.ThrowIfNull(cropName, nameof(cropName));
            ArgumentNullException.ThrowIfNull(seasonName, nameof(seasonName));
            ArgumentNullException.ThrowIfNull(regionName, nameof(regionName));

            try
            {
                logger.LogInformation("Getting Region");
                Region region = unitOfWork.CountriesRepository.GetByRegionName(regionName, cancellationToken).GetAwaiter().GetResult();
                if (region == null)
                    throw new NotFoundException($"Region: ({regionName}) doesn't exist.");
            
                logger.LogInformation("Getting Season");
                Season season = unitOfWork.SeasonRepository.GetBySeasonNameAsync(seasonName, seasonYear, cancellationToken).GetAwaiter().GetResult();
                if (season == null)
                    throw new NotFoundException($"Season: ({season}) doesn't exist.");
            
                logger.LogInformation("Getting Crop");
                Crop crop = unitOfWork.CropRepository.GetCropByName(cropName,cancellationToken).GetAwaiter().GetResult();
                if (crop == null)
                    throw new NotFoundException($"Crop: ({cropName}) doesn't exist.");
            
                logger.LogInformation("Getting SeasonCutOffDate");
                SeasonCutOffDate seasonCutOffDate = unitOfWork.SeasonCutOffDateRepository.GetByCropAndSeason(season.Id, region.Id,crop.CropCategoryId, crop.Id, cancellationToken).GetAwaiter().GetResult();
                if (seasonCutOffDate == null)
                    throw new NotFoundException($"seasonCutOffDate does not exist for this combination of crop, season and region: ({seasonCutOffDate}) doesn't exist.");
            
                logger.LogInformation("Compare cutoff season dates.");
                DateOnly dateToday = DateOnly.FromDateTime(DateTime.Now);
                ValidateApplicationDateResponse response = new ValidateApplicationDateResponse
                {
                    SeasonCutOffDate = seasonCutOffDate.EndDate,
                    StatusCode = "200",
                    
                };
                if (seasonCutOffDate.EndDate.CompareTo(dateToday) > 0)
                {
                    response.IsSeasonClosed = false;
                    response.Message = "The application date is within the cutoff window.";
                }
                else
                {
                    response.IsSeasonClosed = true;
                    response.Message = "The application date is outside of the cutoff window.";
                }
            ...
            catch (Exception e)
            {
                ...
                throw new Exception("Unable to validate season cut off date.");
            }
        }
