...
{
    public static Expression<Func<T, bool>> BuildFilterExpression(Dictionary<string, Common.Filter> filters)
    {
        ...
        foreach (var filter in filters)
        {
            ...
            body = body is ConstantExpression constantExpression && constantExpression.Value.Equals(true)
                ? equality
                : filter.Value.UseOrLogic
                    ? Expression.Or(body, equality)
                    : Expression.AndAlso(body, equality);
        }
    ...
    private static Expression GetProperty(ParameterExpression parameter, string propertyName)
    {
        ...
        foreach (var propName in propertyNames) propertyAccess = Expression.PropertyOrField(propertyAccess, propName);
    ...
    {
        if (targetType == null || string.IsNullOrEmpty(value)) return value;
