...
namespace eSusInsurers.Models.Helpers
{
    ...
    {
        public static Dictionary<string, Models.Common.Filter> CreatePaginationFilters(PagingOptions pageInfo)
        {
            var paginationFilters = new Dictionary<string, Common.Filter>();

            if (pageInfo != null)
            {
                paginationFilters.Add(nameof(pageInfo.Page), CreateFilter("Page", pageInfo.Page?.ToString() ?? "", SearchOperationEnum.Equal));
                paginationFilters.Add(nameof(pageInfo.PageSize), CreateFilter("PageSize", pageInfo.PageSize?.ToString() ?? "", SearchOperationEnum.Equal));
            }
        ...
        public static (int PageSize, int Page) GetPaginationParams(Dictionary<string, Common.Filter> filters)
        {
            ...
            {
                page = int.Parse(filters[key].Value);
            }

            key = "PageSize";
            if (filters.ContainsKey(key))
            {
                pageSize = int.Parse(filters[key].Value);
            }

            return (pageSize, page);
        }

        public static void AddFilterIfNotEmpty(Dictionary<string, Common.Filter> filters, string value, string fieldName, SearchOperationEnum operation)
        {
            if (!string.IsNullOrEmpty(value))
            {
                filters.Add(fieldName, CreateFilter(fieldName, value, operation));
            }
        }

        public static void AddFilterIfValueGreaterThanZero(Dictionary<string, Common.Filter> filters, int? value, string fieldName, SearchOperationEnum operation)
        {
