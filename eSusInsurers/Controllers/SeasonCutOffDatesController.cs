...
namespace eSusInsurers.Controllers
{
    ...
    public class SeasonCutOffDatesController(ISeasonCutOffDateService seasonCutOffDateService) : ControllerBase
    {
        ...
        /// <summary>
        /// CHeck if application is within cutoff date.
        /// </summary>
        /// <remarks>
        /// </remarks>
        [HttpPost("validate_cutoff_date")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> ValidateCutOffDate([FromBody] ValidateApplicationDateRequest request)
        {
            try
            {
                var response = await seasonCutOffDateService.ValidateApplicationDate(request.CropName, request.SeasonName,  request.SeasonYear,request.RegionName, new CancellationToken());
